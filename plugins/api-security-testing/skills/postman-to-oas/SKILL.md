...
Always ask the user for the required inputs, then execute Steps 1–8. Do this
every time the skill is invoked, even if IDE context, open-tabs metadata,
conversation context, or repository search results mention a Postman collection
path. Treat those signals only as possible hints after the user confirms them.
Never infer or select the collection path from open tabs.
...
- **question**:
  ...
  - Postman collection file path (JSON, v2.0 or v2.1)

  - Postman environment file path (optional, JSON)

  - Output file path for the generated OpenAPI spec. Default to `openapi.json` in the collection's directory.
  ```
If the collection path is missing or ambiguous, ask a concise question for the missing path. Ask for the environment path only if the user mentions one but does not provide it.

After collecting inputs, say:
