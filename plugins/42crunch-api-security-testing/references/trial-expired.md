...
`42crunch-setup` completes as a nested workflow. Completing pre-flight this
way naturally carries the calling skill (audit/scan/pipeline) into its
workflow file fresh, so the operation that originally hit the limit gets
retried automatically with the new credentials — no separate "retry" step
needed here.
