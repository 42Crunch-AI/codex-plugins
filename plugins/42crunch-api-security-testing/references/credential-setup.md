...
- **Called as a nested workflow** (caller context is set — e.g. `pre-flight`
  only needed a binary install): keep the existing credentials automatically.
...
- **Called directly by the user**: ask the user directly:
...
>
> 1. Visit **[42Crunch Starter (Free Trial)](https://42crunch.com/freemium/?source=codex)**.
...
**On resume** (user says "continue" or similar): ask the user directly:
