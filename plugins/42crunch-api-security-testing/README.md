...
Automate API security directly in Codex with 42Crunch - audit OpenAPI specs, detect vulnerabilities aligned with OWASP API Security risks (including BOLA/BFLA), and apply AI-powered fixes.
...
- [Codex](https://openai.com/codex) (CLI or app)
- A 42Crunch account — [Starter (Free Trial)](https://42crunch.com/freemium/?source=codex), a paid token-based plan (Individual or Individual Pro), or a Platform account with an API key (Team 10, Team 25, or Enterprise)
...
Codex asks your explicit consent before applying any changes, then re-runs the audit to confirm passage.
...
Runs a live conformance and authorization test against a running API server. Codex confirms the target URL, checks reachability, analyses the OAS (operations, auth schemes, BOLA candidates), and presents a scan preview before any configuration begins. After a happy-path validation run, Codex asks your consent before starting the full fuzzing scan.
...
Codex asks your consent before applying any fixes — both OAS contract updates and server-side code changes.
...
Generates a complete `openapi.json` from an API codebase, a Postman or Insomnia collection, or both. Codex asks which sources are available and uses whichever you provide:
