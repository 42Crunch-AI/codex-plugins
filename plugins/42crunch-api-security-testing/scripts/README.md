...
Scripts are addressed through the resolved absolute `scripts/` directory path so
they resolve regardless of the user's working directory (commands run from the
project git root, not the plugin). `<scripts>` is the plugin's `scripts/` folder
(sibling of `references/`) resolved to an absolute path once in `pre-flight.md`
and substituted literally into every invocation:

```bash
python3 "<scripts>/<name>.py" <args>
```

The script bodies are identical across the Claude, copilot, and codex ports;
only the path placeholder differs per host. On codex the plugin root is not
guaranteed to be exported into a skill's shell, so `pre-flight.md` resolves the
`scripts/` directory to an absolute path once and substitutes it as `<scripts>`.
